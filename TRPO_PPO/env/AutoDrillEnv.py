...
class AutoDrillEnv(gym.Env):
    
    simstates = np.zeros((1,4)) #Continuous state variables
    
    def __init__(self):
        super(AutoDrillEnv, self).__init__()
        ...
        self.stateHigh = np.array([2800, 600, 30, 25])
        
        self.observation_space = spaces.Box(low=self.stateLow, high=self.stateHigh, dtype=np.float64)

        self.X = 2800 #Size in feet of position grid x perimeter
        self.Y = 600 #Size in feet of position grid y perimeter
    ...
    def step(self, action):
        ...
        stepsize=50 #Feet drilled for each step 
    ...
    def reset(self):
        ...
        self.tau = np.random.uniform(low=10, high=30) # 15+-5
        self.ku = np.random.uniform(low=14, high=25)  # 18+-4
